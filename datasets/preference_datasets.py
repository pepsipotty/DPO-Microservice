...
from training.utils import get_local_dir, TemporarilySeededRandom
...
def get_hh(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    dataset = datasets.load_dataset('Anthropic/hh-rlhf', split=split, cache_dir=cache_dir).select(range(10))
    print('done')
...
def get_novalto_dataset() -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    try:
        with open("data/dataset.json", "r") as f:
            raw_data = json.load(f)
    except FileNotFoundError:
        raise ValueError("dataset.json not found in the data directory")
    except json.JSONDecodeError:
        raise ValueError("JSON format problem in dataset.json")
